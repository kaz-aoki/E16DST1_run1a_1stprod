...
bool E16ANA_FieldMapCalibParam::ReadConstantDataCore(int _run_id, std::string _index_file_name) {
  run_id = _run_id;
...
bool E16ANA_FieldMapCalibParam::ReadConstantData(int run_id) {
  auto& calib = E16ANA_CalibDBManager::Instance();
  auto index_file_name = calib.SearchForIndexFileName("fieldmap");
  return ReadConstantDataCore(run_id, index_file_name);
}

bool E16ANA_FieldMapCalibParam::ReadConstantDataByLocal(int run_id, std::string index_file_name) {
  return ReadConstantDataCore(run_id, index_file_name);
}
