...
public:
   ...
   int StripID(int i) { return strip_id[i]; };
...
public:
   ...
   {
      memcpy(fadc_ped_sigma, _fadc_ped_sigma, sizeof(double) * n_strips);
   };
   virtual void SetPedestal(int strip_id, double _fadc_ped) { fadc_ped[strip_id] = _fadc_ped; };
   virtual void SetPedestalSigma(int strip_id, double _fadc_ped_sigma) { fadc_ped_sigma[strip_id] = _fadc_ped_sigma; };
   ...
   */
   ...
   void SetBadPedestalThreshold(double th)
   {
      // ped < -th || th < ped is bad
      bad_pedestal_threshold = th;
   }
   void SetBadPedestalSigmaThreshold(double th)
   {
      // th < sigma is bad
...
protected:
   ...
   double inverted; 
   // If strip order is the opposite direction to the local coordinate, ...
   ...
   virtual int HitClusteringV0(const int min_gap, 
			       const double cluster_threshold);

   virtual int HitClusteringV1();
   virtual int HitClusteringV1(const int min_gap , 
			       const double delta_tdc_threshold);
