...
// Wed Feb 15 12:32:57 2023 by ROOT version 6.18/04
// from TTree tree/tree
// found on file: run0b221021/run020908_ev0-9999.root
//////////////////////////////////////////////////////////

#ifndef GainCalibwoTrack_h
#define GainCalibwoTrack_h
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   GainCalibwoTrack(TTree *tree=0);
   virtual ~GainCalibwoTrack();
...
#ifdef GainCalibwoTrack_cxx
GainCalibwoTrack::GainCalibwoTrack(TTree *tree) : fChain(0) 
{
...
GainCalibwoTrack::~GainCalibwoTrack()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t GainCalibwoTrack::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t GainCalibwoTrack::LoadTree(Long64_t entry)
{
...
void GainCalibwoTrack::Init(TTree *tree)
{
...
Bool_t GainCalibwoTrack::Notify()
{
...
void GainCalibwoTrack::Show(Long64_t entry)
{
...
Int_t GainCalibwoTrack::Cut(Long64_t entry)
{
...
void GainCalibwoTrack::SideHit(int cidside, int ith, double& outadc, double& outt)
{
...
void GainCalibwoTrack::LeftSideHit(int ith, double& outadc, double& outt)
{
  SideHit(1,ith,outadc,outt);
}
void GainCalibwoTrack::RightSideHit(int ith, double& outadc, double& outt)
{
  SideHit(-1,ith,outadc,outt);
}
void GainCalibwoTrack::CalcCrossPoint(E16ANA_GeometryV2* geometry, int lgmid, int lgcid, TVector3& hbd_center)
{
  auto mid2013 = E16ANA_TrackConstant::ModuleID2020To2013_27(lgmid);

  TVector3 o(0., 0., 0.);
  TVector3 g0(0., 0., 0.);
  TVector3 g1(0., 0., 0.);//LG block position
  g1 = geometry->LG(mid2013, lgcid)->GetGPos(o);

  auto hbd_geom = geometry->HBD(mid2013);
  hbd_geom->IsCrossed(g0,g1,hbd_center);

  // std::cout<<"********************"<<std::endl;
  // std::cout<<lgmid<<" "<<lgcid<<std::endl;
  // std::cout<<g1.X()<<" "<<g1.Y()<<" "<<g1.Z()<<std::endl;
  // std::cout<<hbd_center.X()<<" "<<hbd_center.Y()<<std::endl;
  // std::cout<<(hbd_center.X()/100.)+3.<<" "<<(hbd_center.Y()/100.)+3.<<std::endl;

}
void GainCalibwoTrack::CalcCrossPoint(E16ANA_GeometryV2* geometry, int lgmid, int lgcid, int otherlid, int& othermid, TVector3& lcross)
{
  ...
  if( otherlid==4 ){
	...
	if(otherlid==0){
	  trk_geom = geometry->SSD(mid2013t,otherlid-1);
	}
	else{
	  trk_geom = geometry->GTR(mid2013t,otherlid-1);
	}
...
void GainCalibwoTrack::OtherClsLoop(int lid, int xory, int othermid, double lcross, int& nassocs, nearcls& nc, std::vector<double>& outadc, std::vector<double>& outt, std::vector<double>& outlp)
{
  int nclusters;
  std::vector<int>* mid;
...
bool GainCalibwoTrack::wTrgBias(Long64_t entry, int lgmid, int lgcid, int awmin, int awmax, int tw, int& wtrglg, bool& wtrgtrk)
{
  bool wtrg = false;
