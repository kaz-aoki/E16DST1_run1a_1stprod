...
public : 
   ...
   TBranch        *b_rk_hit_gtr300_cogy;
	...
	E16DSTN_PosCorrection(TTree *tree, const char* out_file,  E16ANA_GeometryV2 *_geom, E16ANA_StraightMultiTrack *_fitter);
...
//  out_n_cands = n_selected;
	std::cout << "n_tracks to record " << n_tracks << std::endl;
	for(int i = 0 ; i < n_tracks ; i++){
		std::cout << "i == " << i << std::endl;
		std::cout << "alivedi == " << alive_ids[i] << std::endl;
		
		int tid = alive_ids[i];
	//	std::cout << "mom = " << rk_fit_init_mom_gz->at(tid) << std::endl;
	   std::cout << "chi2 = " << chisq[i] << std::endl;
