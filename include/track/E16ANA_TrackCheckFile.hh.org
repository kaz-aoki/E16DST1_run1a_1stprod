...
//    tree->Branch("hbd_cluster_wotime_cid", &hbd_cluster_wotime_cid);
    ...
    tree->Branch("trg_scaler",               &trg_scaler);
    tree->Branch("trg_nim_scaler",           &trg_nim_scaler);
...
//std::cout << hit.Detector() << " " << lpos.X() << " " << lpos.Y() << std::endl;
    ...
    trg_scaler.resize(E16DST_Constant::NTriggerTypes);
    trg_nim_scaler.resize(E16DST_Constant::NNIMScalersUT3);
...
//      trg_track_lg_t[i] = hit.Timing();
    ...
    for(int i=0;i<E16DST_Constant::NTriggerTypes;++i) {
      trg_scaler[i]     = record.Trigger().ReqScaler(i); 
    }
    for(int i=0;i<E16DST_Constant::NNIMScalersUT3;++i) {
      trg_nim_scaler[i]     = record.Trigger().NimScaler(i); 
    }
    return;
  }
...
//    sim_track_detected.clear();
    ...
    x_rough_refit_chi_square.resize(rn_cands);
#ifdef TRACK_FIND_WO_TARGET
    ...
    rk_hit_gtr100_xt5.resize(rn_cands);
    rk_hit_gtr100_yt5.resize(rn_cands);
    rk_hit_gtr200_xt5.resize(rn_cands);
    rk_hit_gtr200_yt5.resize(rn_cands);
    rk_hit_gtr300_xt5.resize(rn_cands);
    rk_hit_gtr300_yt5.resize(rn_cands);
    ...
    for (int i = 0; i < rn_cands; ++i) {
      ...
      x_rough_refit_chi_square[i] = cand.ReXChiSquare();
#ifdef TRACK_FIND_WO_TARGET
	...
	rk_hit_ssd_gx[i]   = E16DST_DST1Constant::kInvalidValue;
      ...
      rk_hit_gtr100_xt5[i] = gtr100_xclst->Timing5();
      rk_hit_gtr100_yt5[i] = gtr100_yclst->Timing5();
...
//241022 mtomoki
      ...
      */
      ...
      rk_hit_gtr200_xt5[i] = gtr200_xclst->Timing5();
      rk_hit_gtr200_yt3[i] = gtr200_yclst->Timing3();
      rk_hit_gtr200_yt4[i] = gtr200_yclst->Timing4();
      rk_hit_gtr200_yt5[i] = gtr200_yclst->Timing5();
...
//241022 mtomoki
      ...
      */
      ...
      rk_hit_gtr300_xt5[i] = gtr300_xclst->Timing5();
      rk_hit_gtr300_yt3[i] = gtr300_yclst->Timing3();
      rk_hit_gtr300_yt4[i] = gtr300_yclst->Timing4();
      rk_hit_gtr300_yt5[i] = gtr300_yclst->Timing5();
...
//  std::vector<std::vector<bool>>   trg_track_hbd_is_t_match;
  ...
  std::vector<uint32_t> trg_scaler;
  std::vector<uint32_t> trg_nim_scaler;
#ifdef TRACK_EFF_CHECK
  int                 mock_n_tracks;
