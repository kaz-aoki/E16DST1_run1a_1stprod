...
void E16ANA_CTBasic::SetMap(){
  //unordered_map<string, ch_pp*> mapdata;

        E16ANA_CalibDBManager& calib=E16ANA_CalibDBManager::Instance();
	FILE* fp_map = calib.CalibFileOpenBinary("CT-specmap", calib.CurrentRunID() );
	if ( fp_map==NULL ) {
		std::cerr<<"CT spec map file is not found !"<<std::endl;
		exit(1);
	}
	...
	*specpoint=spec;
	...
	if(!(0<=mod&&mod<110&&0<=blk&&blk<60&&0<=key_ip&&key_ip<100)){
	  ...
	  ctspecmap[mod][blk] = *specpoint;
	  ctspecmap_ip[key_ip] = *specpoint;
	}
	//delete specpoint;
	}//while loop
	fclose(fp_map);
...
E16ANA_CTBasic::ch_pp* E16ANA_CTBasic::GetSpec(uint16_t module, uint16_t block){
  E16ANA_CTBasic::ch_pp* spec = &ctspecmap[module][block];
  return spec;
}

E16ANA_CTBasic::ch_pp* E16ANA_CTBasic::GetSpec(int ip){
  E16ANA_CTBasic::ch_pp* spec = &ctspecmap_ip[ip];
  return spec;  
}
