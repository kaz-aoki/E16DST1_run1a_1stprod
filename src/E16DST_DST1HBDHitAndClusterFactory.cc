...
#include "E16ANA_HBDCalibaration.hh"
#include ""
...
int E16DST_DST1TriggerHitAndClusterFactory(E16DST_DST0Detector<E16DST_DST0HBDHit>& hits0,
					   E16ANA_HBDCalibration *hbd_calib,
					   E16DST_DST1Detector<E16DST_DST1HBDHit>* hits1,
					   E16DST_DST1Detector<E16DST_DST1HBDCluster>* clusters1)
{
  //----------- const
  ...
  //----------- get cut criteria
  std::string hbd_cut = calib.CalibFileName("HBD-cut", 0);
  E16ANA_HBDCut hbd_cut;
  hbd_cut.ReadFile(hbd_cut);
  ...
  std::vector<HBD_ClusterAnalysis> cs;
  cs.resize(n_modules);
  ...
  for (int ith_dst0hit = 0; ith_dst0hit < hits0.NumberOfHits(); ith_dst0hit++){
    E16DST_DST0HBDHit dst0_hit = hits0.Hit(ith_hit);
    ...
    if(is_dst0hit){
      E16DST_DST1HBDHit &dst1_hit = hits1->Hit(dst1_hid);
      ...
      dst1_hit->SetPeakHight(pe);//peak should be expressed in units of p.e.
      dst1_hit->SetTiming(timing);//50% of maximum peak height
      dst1_hit->SetLocalPos(HBD_Geometry::GetPadLocalCOG(mid, pid));
  ...
  for(auto p : cs){
    ...
    p.GetClusters(clusters);
    for(auto cl : clusters){
      double e_prob = 0.;
      E16DST_DST1HBDCluster &dst1_cl = clusters1->Hit(dst1_cid);
      dst1_cl.SetInvalid();
      ...
      TVector3 lpos(cl.lpos_w_pe[0], cl.lpos_w_pe[1], cl.lpos_w_pe[2]);
