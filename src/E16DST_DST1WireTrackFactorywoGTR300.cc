...
int E16DST_DST1WireTrackFactory3DwoGTR300(E16DST_DST0PhysicsEvent *event0, E16DST_DST1Detector<E16DST_DST1SSDHit, E16DST_DST1SSDCluster> *ssd1, E16DST_DST1Detector<E16DST_DST1GTRHit, E16DST_DST1GTRCluster> *gtr1, std::vector<std::shared_ptr<E16DST_DST1StraightTrack3D>> &st_tracks, E16ANA_GTRcalibPedestal &gtrped){
	static bool isFirst = true;
	static StraightTrackAnalyzerOfWireV1woGTR300 *straight_analyzer;
...
//		geom = new E16ANA_GeometryV2(geomName);
		...
		if(targets.IsWire()){
			...
			if(x1 < 0){
				straight_analyzer = new StraightTrackAnalyzerOfWireV1woGTR300(2, 1, x1 ,z1, x2, z2);//track selected only by GTR : first arg is n of tgt, second is plus or minus, this is opposite with x1 sign cuz arg is based on beam coordinate
			}
			else if(x1 > 0){
				straight_analyzer = new StraightTrackAnalyzerOfWireV1woGTR300(2, -1, x1 ,z1, x2, z2);//track selected only by GTR : first arg is n of tgt, second is plus or minus. this is oposite with x1 sign
