...
E16ANA_GTRStripAnalyzer::E16ANA_GTRStripAnalyzer(int _n_strips, int _n_sampling)
   : n_strips(_n_strips), n_sampling(_n_sampling), 
     strip_pitch(0.35), gem_threshold(4.0), gem_tot_threshold(-10000.0),
     drift_velocity(0.010), drift_gap_center(0.0), fadc_clock_period(25.0), 
     fadc_t0_correction(0.0),
     gem_tdc_min(-10000.0), gem_tdc_max(10000.0), gem_tr(75.0), 
     threshold_fraction(0.5), inverted(1.0),
     cluster_minimum_gap(2), cluster_delta_tdc(10000.0),  
...
int E16ANA_GTRStripAnalyzer::HitClusteringV1(const int min_gap, const double delta_tdc_threshold)
{
   ...
   //std::cout << "gap: " << min_gap << ", tdc_threshold: " 
   //	     << delta_tdc_threshold << std::endl;
...
void E16ANA_GTRStripAnalyzer::CalcWaveParamsPeak()
{
   ...
   for (int i = 0; i < n_strips; i++) {
     ...
     //std::cout << "strip ID: " << i << ", adc: " << fadc[i][0] << std::endl; 
