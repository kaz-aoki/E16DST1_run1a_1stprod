...
E16ANA_SSDStripAnalyzer::E16ANA_SSDStripAnalyzer(int _n_strips, int _n_sampling)
   : n_strips(_n_strips), n_sampling(_n_sampling), strip_pitch(0.08), ssd_threshold(1.0), ssd_tot_threshold(-10000.0),
//     drift_velocity(0.010), drift_gap_center(0.0), fadc_clock_period(25.0), fadc_t0_correction(0.0),
     ...
     std_threshold(10.0),lower_limit(100), upper_limit(950)//Takaura cut
   {
...
void E16ANA_SSDStripAnalyzer::GetFadc(double (&fadc)[768][8], int strip_id, int16_t *waveform){
   for (int j = 0; j < 8; j++) {
      fadc[strip_id][j] = waveform[j] - fadc_ped[strip_id];
    }
...
void E16ANA_SSDStripAnalyzer::CalcWaveParamsPeak(int ch, double t_cutoff) {
   ...
   if (ch < n_strips) {
      ...
      ave /= n_sampling;
      for (int j = 0; j < n_sampling; j++) {
         var += (ave - fadc[ch][j])*(ave - fadc[ch][j]);
...
//      std::cout << sqrt(var)/n_sampling << std::endl;
      if(sqrt(var)/n_sampling > std_threshold){// Takaura cut
//         std::cout <<  "std = " << sqrt(var)/n_sampling << std::endl;
         fadc_peak[ch] = -255.0;
         peak_count = -1;
      }
...
//         peak_count = -1;
//      } 
         else if(fadc_peak[ch] < lower_limit || fadc_peak[ch] > upper_limit){ //Takaura cut
//         std::cout << "peak = " << fadc_peak[ch] << std::endl;
         fadc_peak[ch] = -255.0;
         peak_count = -1;
      }
         else {
