...
E16ANA_StraightMultiTrack::E16ANA_StraightMultiTrack(E16ANA_GeometryV2 *_goem,  std::vector<TVector3> &_tgt_pos, int _n_tracks) : 
...
void E16ANA_StraightMultiTrack::Clear(){
   for(int i=0; i<n_tracks; i++){
...
void E16ANA_StraightMultiTrack::SetCharge(int track_id, double charge){
   if(track_id >= n_tracks){
      std::cout << "E16ANA_StraightMultiTrack::SetCharge() : Error."
...
void E16ANA_StraightMultiTrack::SetInitialMomentum(int track_id, const TVector3 &mom){
   if(track_id >= n_tracks){
      std::cout << "E16ANA_StraightMultiTrack::SetInitialMomentum() : Error."
                << " TrackID = " << track_id << std::endl;
      return;
   }
...
void E16ANA_StraightMultiTrack::AddHit(int track_id, int layer_order,
      const E16ANA_DetectorGeometry *_geom,
      const TVector3 &lpos, const TVector3 &sigma){

   if(track_id >= n_tracks){
      std::cout << "E16ANA_StraightMultiTrack::AddHit() : Error."
                << " TrackID = " << track_id
                << std::endl;
      return;
   }
...
/* Equation Of Motion */
void E16ANA_StraightMultiTrack::EOM(double mom, double r[], double dr[], double d2r[]){
   int jj[3]={1,2,0};
   int kk[3]={2,0,1};
   double point[3];
...
void E16ANA_StraightMultiTrack::rkn4(double mom, double y[], double dydx[], double h,
...
double E16ANA_StraightMultiTrack::RungeKuttaTracking(int track_id, double dstp, double mom, double r0[], double dr0[]){
...
int E16ANA_StraightMultiTrack::RungeKuttaTrackingToNextHit(int track_id, LayerHits_t &next_layer_hits, double dstp, double mom,
   ...
   while(1){
      ...
      if(is_crossed){
         ...
         if(print_level >= 1){
            std::cout << "E16ANA_StraightMultiTrack::RungeKuttaTrackingToNextHit : Out of tracking region." << std::endl;
         }
         break;
//      }else if(n_steps > 10000){
      }else if(n_steps > max_steps){
         if(print_level >= 1){
            std::cout << "E16ANA_StraightMultiTrack::RungeKuttaTrackingToNextHit : Excess max number of steps." << std::endl;
         }
         break;
      }
...
#if 0
bool E16ANA_StraightMultiTrack::IsCrossed(Plane_t &plane, double r0[], double r1[], TVector3 &cross_point){
...
void E16ANA_StraightMultiTrack::PreConditioning(){
...
//double E16ANA_StraightMultiTrack::Fit(bool vertex_xy_fixflag, bool pyfixflag, bool vertex_z_fixflag){
double E16ANA_StraightMultiTrack::Fit(bool vertex_xy_fixflag, bool pyfixflag, bool vertex_z_fixflag, int _strategy, int _max_function_calls){
   PreConditioning();
   ...
   E16ANA_StraightMultiTrackFunction func(this);
...
//   printf("vertex init %f %f %f \n", vertex_init.X(), vertex_init.Y(), vertex_init.Z());
...
//   printf("minuit %f %f %f ", minuit->X()[0], minuit->X()[1], minuit->X()[2]);
   ...
   for(int i=0; i<n_tracks; i++){
      momentum_fit[i].SetXYZ(
			     cos(minuit->X()[i*3+4])*cos(minuit->X()[i*3+3]),
			     sin(minuit->X()[i*3+4]),
			     cos(minuit->X()[i*3+4])*sin(minuit->X()[i*3+3])
            );
   }

   //printf("fit %f %f %f %f \n",momentum_fit[0].X(),momentum_fit[0].Y(),momentum_fit[0].Z(),chisq  );
...
double E16ANA_StraightMultiTrack::RungeKuttaTracking(int track_id, const TVector3 &vertex, const TVector3 &momentum, double charge){
   ...
   return RungeKuttaTracking(track_id, dstp, mom, r0, dr0);
}

double E16ANA_StraightMultiTrack::MinuitFunction(const double *pars){
   double chisq = 0.0;
   vertex_fit.SetXYZ(pars[0], pars[1], pars[2]);
   for(int i=0; i<n_tracks; i++){
     //momentum_fit[i].SetXYZ(pars[i*3+3], pars[i*3+4], pars[i*3+5]);
     momentum_fit[i].SetXYZ(cos(pars[i*3+4])*cos(pars[i*3+3]), sin(pars[i*3+4]), cos(pars[i*3+4])*sin(pars[i*3+3]));
...
void E16ANA_StraightMultiTrack::PrintHits(LayerHits_t &layer){
...
void E16ANA_StraightMultiTrack::PrintHits(){
   std::cout << "E16ANA_StraightMultiTrack::PrintHits() : # of tracks = " << n_tracks << std::endl;
   for(int i=0; i<n_tracks; i++){
...
ROOT::Math::IMultiGenFunction* E16ANA_StraightMultiTrackFunction::Clone() const {
   return new E16ANA_StraightMultiTrackFunction(this->multi_track);
}
