...
void E16ANA_StraightStepTrack::RKN_1stepBockArray(Hep3Vector& iy, Hep3Vector& idy, 
					  double x, 
               double h, Hep3Vector& yResult,Hep3Vector& dyResult
	//	void (E16ANA_StraightStepTrack::*f)(double, double*, double*, double*)
...
//  EquationOfMotion( x+k, y3, dy3, k4);
  //  (*f)( x+k, y3, dy3,  k4);
  yResult  =Hep3Vector( y[0] + h*( dy[0]  ),
                        y[1] + h*( dy[1]  ) ,
                        y[2] + h*( dy[2]  )
                        );
                                   
  dyResult =Hep3Vector( dy[0]  ,
                        dy[1]  ,
                        dy[2]  
                        );
//  
...
void E16ANA_StraightStepTrack::EquationOfMotion(
...
//  double constant=0.299792458*1e-6; //[?]  kawama FMField
  double constant=0.0 ;//no magnetic field
...
int E16ANA_StraightStepTrack::TraceInMagnet( 
...
const Hep3Vector E16ANA_StraightStepTrack::errorVector = Hep3Vector(-99999,-99999,-99999);

E16ANA_StraightStepTrack::E16ANA_StraightStepTrack( 
...
#if 1
    int flag1=TraceInMagnet(  stepSize, nStep, initPoint, initMom,
			     charge,  pointData1, momData1);
    int flag2=TraceInMagnet(  stepSize, nStep, initPoint, initMom2,
...
Hep3Vector E16ANA_StraightStepTrack::PointAtR(double r){
...
Hep3Vector E16ANA_StraightStepTrack::ArrayGet(int    step,
...
Hep3Vector E16ANA_StraightStepTrack::ArrayGet(double step,
			     Hep3Vector* array1,Hep3Vector* array2) {
//  if( step>arraySize ) { return errorVector;}
...
//--------------------------------------------------------------
static E16ANA_StraightStepTrack* crossSt1;
static E16ANA_StraightStepTrack* crossSt2;//for cross calculation.

int E16ANA_StraightStepTrack::Cross(E16ANA_StraightStepTrack& st, double* distance,
...
int E16ANA_StraightStepTrack::CrossXZ(E16ANA_StraightStepTrack& st, double* distance,
		     double* step1, double* step2) {
...
int E16ANA_StraightStepTrack::fcnCalledCount=0;
int E16ANA_StraightStepTrack::XZdistanceFlag=0;

int E16ANA_StraightStepTrack::MinuitDistanceXZ( double* stepSize, double* lowLimit,double* upLimit,   double* distance,double* param,double* error) {
...
int E16ANA_StraightStepTrack::MinuitDistance( double* stepSize, double* lowLimit,double* upLimit,   double* distance,double* param,double* error) {
...
(  void(*) (const int *npar, double *grad, double *fval, const double *xval,
	      const int *iflag, double (*futil)())  )
    E16ANA_StraightStepTrack::CalcDistance;
  ...
  minuit->SetFCN(      & E16ANA_StraightStepTrack::CalcDistance );
...
int E16ANA_StraightStepTrack::CrossZconstPlane(double iz, Hep3Vector& crossPoint,
Hep3Vector& crossMom){
...
int E16ANA_StraightStepTrack::CrossXconstPlane(double ix, Hep3Vector& crossPoint,
Hep3Vector& crossMom){
...
int E16ANA_StraightStepTrack::CrossXconstPlane(double ix, vector<Hep3Vector>* crossPoint, vector<Hep3Vector>* crossMom){
  crossPoint->clear();
  crossMom->clear();
...
//------------------------------------------------------------
#if 1
void E16ANA_StraightStepTrack::CalcDistance(int & npar,double *grad, double & fval,
				    double *xval, int iflag){
#else
void E16ANA_StraightStepTrack::CalcDistance(const int *npar, double *grad,
