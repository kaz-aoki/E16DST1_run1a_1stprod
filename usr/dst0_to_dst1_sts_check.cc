///////
//
// std0_to_dst1_sts_check based on lg_check.
// many lg related things are left untouched.
//
//
///////
...
int main(int argc, char* argv[]) {
  ...
  TTree* trgtree = new TTree("trgtree","trgtree");
  ...
  TString br_uint64 = "/l";
  TString br_int32 = "/I";
  trgtree->Branch("l1_trgtype",&l1_trgtype,"l1_trgtype"+br_int16);
  trgtree->Branch("trgtyp_rcv_count" ,&trgtyp_rcv_count ,"trgtyp_rcv_count"+br_uint32);
  trgtree->Branch("trgdata_rcv_count",&trgdata_rcv_count,"trgdata_rcv_count"+br_uint32);
  trgtree->Branch("trglocal_count",&trglocal_count,"trglocal_count"+br_uint32);
  trgtree->Branch("l1_geritimestamp",&l1_geritimestamp,"l1_geritimestamp"+br_uint64);
  trgtree->Branch("gbt",&gbt,"gbt"+br_int16);
  trgtree->Branch("port",&port,"port"+br_int16);
  trgtree->Branch("trgtype",&trgtype,"trgtype"+br_int16);
  trgtree->Branch("trg_num_emu2geri",&trg_num_emu2geri,"trg_num_emu2geri"+br_int16);
  trgtree->Branch("ut3timestamp",&ut3timestamp,"ut3timestamp"+br_uint64);

  trgtree->Branch("ut3spill",&ut3spill,"ut3spill"+br_int32);
