...
//  E16ANA_WaveformFitter *wf1d_fitter = new E16ANA_WaveformFitter(hbd_waveform_template);
  ...
  outtree->SetBranchAddress("rk_fit_init_mom_gz",&rk_fit_init_mom_gz);
  outtree->SetBranchAddress("rk_fit_init_pos_gx",&rk_fit_init_pos_gx);
  ...
  for(int i = 0 ; i<modules.size(); i++){
    str.Form("hist_residual_%d",modules[i]);
...
//      check_file.FillTree();
	...
	*/
	...
	double global[3] = { rk_fit_init_pos_gx->at(i),
			     rk_fit_init_pos_gy->at(i),
			     rk_fit_init_pos_gz->at(i)};
	double global2[3] = { global[0] + mom[0]*1000.,
			      global[1] + mom[1]*1000.,
			      global[2] + mom[2]*1000. };
	display.DrawLine(global,global2);
...
//      dst1->WriteAnEvent(event0);
  ...
  check_file.cd();
  for( auto& hist: hist_res_map ){
    hist.second->Write();
    std::cout << hist.first << "   " << hist.second->GetTitle() << std::endl;
  }
