...
void GTRResidualCalculator::SelectTrack(int track_index, std::array<std::vector<int>,
	track_const::kNumTrackingLayers> *used_cluster_ids){
...
void GTRResidualCalculator::RKAnalyze(E16ANA_MultiTrack *fitter){
	...
	for(int i = 0; i < selected_track_indexs.size(); i++){
		...
		std::cout << "m id = " << track_ssd_cluster_mid->at(tid) << std::endl;
		...
		hit_lpos[0] = geometry->SSD(track_const::ModuleID2020To2013(track_ssd_cluster_mid->at(tid)))->GetLPos(hit_gpos[0]);
		hit_lpos[1] = geometry->GTR(track_const::ModuleID2020To2013(track_gtr100_cluster_mid->at(tid)), 0)->GetLPos(hit_gpos[1]);
		hit_lpos[2] = geometry->GTR(track_const::ModuleID2020To2013(track_gtr200_cluster_mid->at(tid)), 1)->GetLPos(hit_gpos[2]);
		hit_lpos[3] = geometry->GTR(track_const::ModuleID2020To2013(track_gtr300_cluster_mid->at(tid)), 2)->GetLPos(hit_gpos[3]);
//		int removed_layer = -1;
...
// --- // 
		...
		for(int l=0; l < 4; l++){
            ...
            if(l ==1 ){pre_detector_res_x[l]  = rk_res_gtr100_x->at(tid);}
            if(l ==2 ){pre_detector_res_x[l]  = rk_res_gtr200_x->at(tid);}
            if(l ==3 ){pre_detector_res_x[l]  = rk_res_gtr300_x->at(tid);}
            if(l ==1 ){pre_detector_res_y[l]  = rk_res_gtr100_y->at(tid);}
            if(l ==2 ){pre_detector_res_y[l]  = rk_res_gtr200_y->at(tid);}
            if(l ==3 ){pre_detector_res_y[l]  = rk_res_gtr300_y->at(tid);}
		}
            trk_charge             = charge;
			pre_chi_square         = chi_square->at(tid);//rk 4 layers
		    module_id[4]           = track_hbd_mid->at(tid);
		    module_id[5]           = track_lg_mid->at(tid);
		    //lg_t                   = track_lg_hit_t->at(tid);
...
// begin three layer fit including tgt, and four layer w/o tgt (removed i == 4)
	    for(int removed_layer_i = 0; removed_layer_i < 5; removed_layer_i++){
				...
				fitter->Clear();
				fitter->SetInitialVertex(TVector3(0.,0., tgt_zs[min_id]), kTargetSigma);
				fitter->SetCharge(0, charge);//first argument is n of track (now, single )
				fitter->SetInitialMomentum(0, moms);
				for(int l=0; l < 4; l++){
					auto mid2013 = track_const::ModuleID2020To2013(mids[l]);
...
//				std::cout << "removed l , res  " <<removed_layer_i << ", " <<  v_tmp_lres[0][0].X() << std::endl;
				...
				post_3l_detector_res_y[removed_layer_i][ll] = v_tmp_lres[0][0].Y();
					...
					fitter->Clear();
					fitter->SetInitialVertex(TVector3(0.,0., tgt_zs[min_id]), zero_sigma);
					fitter->SetCharge(0, charge);//first argument is n of track (now,single )
					fitter->SetInitialMomentum(0, moms);
			    	for(int l=0; l < 4; l++){
					auto mid2013 = track_const::ModuleID2020To2013(mids[l]);
