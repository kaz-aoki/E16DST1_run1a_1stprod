...
int main(int argc, char* argv[]){
   if (argc != 7) {
      ...
      cerr << "./bin [input_gtr.dst0] [output.dst1] [run_num] [max physics event (all: -1)] [ removed_layer (-1 = all, 0, ssd, 1,2,3 = gtr)] [input_lg.dst0]" << endl;
      return 1;
    }
...
//for lg dst0
	 string in_file_lg  = argv[6];
	 auto dst0_lg = new E16DST_DST0();
    if (!dst0_lg->Open(argv[6], E16DST_DST0::ReadMode)) {
      std::cout << "failed to open file : " << argv[6]  <<std::endl;
      exit(1);
  //    return 1;
    }
...
//For Run0d
		while (dst0_lg->ReadAnEvent()) {
		auto event0_lg = dynamic_cast<E16DST_DST0PhysicsEvent*>(dst0_lg->Event());
      auto eventlg_type = dst0_lg->EventType();
      if (eventlg_type != E16DST_DST0EventType::Physics) {
      	std::cout << "Event ID = " << dst0_lg->Event()->EventID() << " is not Physics Event, Event Type = " << event_type << std::endl;
			continue;
		}
		int event_lgid = event0_lg->EventID();
		if(event_lgid < event_id){ continue;} 
		else if (event_lgid > event_id) {continue;}
		else if (event_lgid == event_id ){
			auto& lg_hits0 = event0_lg->LG(); 
			auto& trigger_gtr_hits0 = event0->TriggerGTR();
			auto& trigger_hbd_hits0 = event0->TriggerHBD();
			auto& trigger_lg_hits0  = event0->TriggerLG();
			...
			E16DST_DST1TriggerFactory(trigger_param, event0->TriggerGTR(), event0->TriggerHBD(), event0_lg->TriggerLG(), event0_lg->UT3(), &record.Trigger());
...
// Track
		check_file.AddRecord(*geometry, event0->EventID(), event0->SpillID(), event0_lg->TimeStampInSpill(), event0_lg->UT3().TriggerTime() % 8 , record);
