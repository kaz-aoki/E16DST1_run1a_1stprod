...
int main(int argc, char* argv[]) {
  ...
  if ( argc!=4 && argc!=5 ) {
    std::cerr << "./bin [layer id] [outfile] [maxevent]" << std::endl;
    return -1;
  }
  auto lid = stoi(argv[1]);
  ...
  if(argc==4){
    ...
    // in_chain->Add("/ccj/u/nakasuga/work/dst1ana/track/out/wotrack/v5/*.root");
  }
  else if(argc==5){
  ...
  t->LGADCGTRymix(lid,out_file_name, maxevent);
