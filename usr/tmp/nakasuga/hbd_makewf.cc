...
//  int run_num;
  ...
  ////
  std::ifstream fin("./hbdwf.txt");
  std::vector<int> as_event_id;
  ...
  while (dst0->ReadAnEvent()) {
    ...
    if (event_type == E16DST_DST0EventType::Physics) {
      ...
      ////
      bool is_ana_event = false;
      int ana_index = -10000;
      for(int iwf=0;iwf<as_event_id.size()-1;iwf++){
	if( event_id==as_event_id.at(iwf) ){
	  is_ana_event = true;
	  ana_index = iwf;
	  break;
	}
	...
	////
	double tlx = cluster.LocalPos().X();
	double tly = cluster.LocalPos().Y();
	if( fabs(tlx-as_c_lx.at(ana_index))<0.001 && fabs(tly-as_c_ly.at(ana_index))<0.001 ){
	  std::cout<<event_id<<" "<<cluster.ClusterId()<<" "<<cluster.ClusterSize()<<std::endl;
	}
	else{
	  continue;
	}
	...
	////
	bool is_hitm = false;
	for(int ihm=0;ihm<c_hitmembers.at(0).size();ihm++){
	  if( c_hitmembers.at(0).at(ihm)==hit.HitId() ){
	    is_hitm = true;
	  }
	}
	if(!is_hitm){
	...
	for(int ith_dst0hit =0; ith_dst0hit < hbd_hits0.NumberOfHits(); ith_dst0hit++){
      ...
      // h_wf.resize(n_hits);
      h_wf.resize(c_hitmembers.at(0).size());////
