...
void track_analyzer_220715::MakeBranches(TTree* tree) {
  ...
  tree->Branch("ppi_mass",                   &out_pip_mass);
...
int track_analyzer_220715::BestTargetID(int n, int* good_tgt_id_set, vector<double>* good_tgt_ids) {
  *good_tgt_id_set = 0;
  good_tgt_ids->clear();
...
void track_analyzer_220715::ParentInfo(int n) {
  ...
  out_fit_parent_best_tgt_id[n] = BestTargetID(n, &out_fit_parent_good_tgt_id_set[n], &out_fit_parent_good_tgt_ids[n]);
  return;
}
