...
int main (int argc, char** argv) {
	if(argc != 7){
		std::cout << "./bin/~~ [input.root] [output.root] [start_ev] [end_ev] [residual_layer] [pdf_name]" << std::endl;
		return 0;	
	}
...
//	TTree *tree          = (TTree*)fin->Get("outtree");
	...
	rt->DrawHist(tree, start_event, end_event,  print_cycle, residual_layer, pdf_name);
