...
int main (int argc, char** argv) {
	if(argc != 6){
		std::cout << "./bin/~~ [inputdir (No need last slash )] [output.root] [st_event] [end_event] [pdf_name]" << std::endl;
		return 0;	
	}
   const char *dirname = argv[1];
	std::string out_file = argv[2];
	int event_start      = atoi(argv[3]);
	int event_end       = atoi(argv[4]);
...
//	E16DSTN_ReadStraightTree *rt = new E16DSTN_ReadStraightTree(chain, out_file.c_str());//initialize
	E16DSTN_AfterTrackAna *ana = new E16DSTN_AfterTrackAna(chain, out_file.c_str());//initialize
	int nevent           = chain->GetEntries();
	int print_cycle      = 2000;
	ana->DrawHist(chain, print_cycle, event_start, event_end, pdf_name);
	return 0;	
}
